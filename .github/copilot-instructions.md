...
- **Pi 5 / libgpiod Note**: On Raspberry Pi 5 (Debian 12 Bookworm / 13 Trixie), `node-dht-sensor` must be compiled with `--use_libgpiod=true`. See README for setup instructions.
...
The DHT sensor setup uses `node-dht-sensor` with a per-sensor polling interval. Each sensor (identified by its GPIO pin number) gets its own `setInterval` timer.

Key notes:
- Sensor type: `11` for DHT11, `22` for DHT22/AM2302/AM23xx
- Always capture `type` and `gpio` in the closure — do NOT iterate a shared dictionary inside the interval
- Use the corrected `pollInterval` (not the raw config value) when calling `setInterval`
- For Raspberry Pi 5, `node-dht-sensor` must be compiled with `--use_libgpiod=true`

```javascript
// Correct pattern: per-sensor interval with captured variables
const sensorLib = require('node-dht-sensor');
sensorLib.initialize(type, gpio);

const capturedGpio = gpio;
const capturedType = type;
setInterval(() => {
    sensorLib.read(capturedType, capturedGpio, async function (err, temperature, humidity) {
        if (err) {
            adapter.log.error(`Failed to read DHTxx on GPIO ${capturedGpio}: ${err}`);
        } else {
            adapter.log.debug(`Read DHTxx GPIO ${capturedGpio}: ${temperature}°C, ${humidity}%`);
            await adapter.setStateChanged(`gpio.${capturedGpio}.temperature`, temperature, true);
            await adapter.setStateChanged(`gpio.${capturedGpio}.humidity`, humidity, true);
        }
    });
}, pollInterval);
```
