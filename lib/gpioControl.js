                    ...
                    watch.on('change', (value) => {
                        ...
                        if (this.adapter.config.gpios[port].input === 'in') {
                            ...
                            if (this.debounceTimers[port] !== null) {
                                this.log.debug('Debounce was not null:' + this.debounceTimers[port]);
                                ...
                                this.log.debug('Deboumce timer cleared.');
                            } else {
                                ...
                                this.debounceTimers[port] = this.adapter.setTimeout(async (t_port, t_value) => {
                                    ...
                                    await this.readValue(t_port, t_value);
                                }, this.config.inputDebounceMs, port, value);
                                this.log.debug(`Debounce timer started for port ${port} with value ${value} for ${this.config.inputDebounceMs}ms.`);
                            }
                        }
                    });
                }
