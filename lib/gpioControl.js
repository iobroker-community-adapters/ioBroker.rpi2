...
class GpioControl {
    ...
    async setupGpio(gpioPorts, buttonPorts) {
        ...
        try {
            ...
            try {
                if (typeof opengpio.info === 'function') {
                    this.log.debug('Info: ' + opengpio.info());
                } else {
                    this.log.debug(opengpio);
                }

                const {stdout, stderr} = await exec('cat /proc/device-tree/model');
                this.log.debug('CPU Info: ' + stdout);
                this.log.debug('STDERR: ' + stderr);
                const deviceString = parseModelStringToClassName(stdout);
                this.log.debug(`Got ${deviceString} from ${stdout}.`);
                this.gpioChip = opengpio[deviceString];
                this.log.debug(this.gpioChip);
                if (!this.gpioChip) {
                    this.log.error(`Cannot find GPIO chip for device: ${deviceString} generated from model string ${stdout}. Please report.`);
                    return;
                }
            } catch (e) {
            ...
            if (this.gpioChip) {
                ...
                this.gpioPortsRequest.on('change', (line, value) => {
                    this.log.debug(`Listener for port ${line} got value ${value}`);
                });
