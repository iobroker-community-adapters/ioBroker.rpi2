...
class GpioControl {
    ...
    setupGpio(gpioPorts, buttonPorts) {
        ...
        try {
            ...
            this.numberOfPins = this.ghipChip.getNumberOfLines();
            this.log.debug(`GPIO chip ${this.gpioChip.getChipName()}, ${this.gpioChip.getChipLabel()} with ${this.gpioChip.getNumberOfLances()} PINs initialized`);
        } catch (e) {
            this.gpioChip = null;
                ...
                if (direction === 'in') {
                    ...
                    try {
                        ...
                        this.gpioPorts[port].requestInputModeFlags(`ioBroker.${this.adapter.namespace}.${port}`, this.config.inputPullUp ?
                            LineFlags.GPIOD_LINE_REQUEST_FLAG_BIAS_PULL_UP : LineFlags.GPIOD_LINE_REQUEST_FLAG_BIAS_DISABLE); //if no pull up, deactivate resistors. Is that correct? Or do we want pull down, then?
                        this.readValue(port);
            ...
            if (this.gpioInputPorts.length > 0) {
                ...
                this.gpioInputPortsHandler = this.adapter.setInterval(() => {
                    for (const port of this.gpioInputPorts) {
                        this.readValue(port.getLineOffset());
                    }
