    ...
    async setupGpio(gpioPorts, buttonPorts) {
        ...
        try {
            ...
            if (this.gpioChip) {
                ...
                // Each watched input permanently occupies one libuv worker thread (see the
                // uv_queue_work endless loop in opengpio). If more inputs are watched than the
                // thread pool can hold, the surplus ports never receive edge events and thus
                // never update on their own. main.js raises UV_THREADPOOL_SIZE to avoid this;
                // warn loudly if we still risk exceeding it.
                const threadPoolSize = Number(process.env.UV_THREADPOOL_SIZE) || 4;
                this.log.debug(
                    `Watching ${this.gpioInputPorts.length} input port(s); UV_THREADPOOL_SIZE=${threadPoolSize}.`,
                );
                if (this.gpioInputPorts.length >= threadPoolSize) {
                    this.log.warn(
                        `Configured ${this.gpioInputPorts.length} GPIO input port(s), but only ${threadPoolSize} libuv worker threads are available. ` +
                            `Some inputs may never update on their own (see https://github.com/iobroker-community-adapters/ioBroker.rpi2/issues/378). ` +
                            `Set the UV_THREADPOOL_SIZE environment variable to at least ${this.gpioInputPorts.length + 4} and restart the adapter.`,
                    );
                }
